...
from kc.cli.plugins.site_functions import logwatch
...
def kc_debug_hook(app):
    pass


class KCDebugController(CementBaseController):
    class Meta:
        label = 'debug'
        ...
        usage = "kc debug [<site_name>] [options] "
    ...
    def debug_nginx(self):
            ...
            for ip_addr in debug_address:
                if not ("debug_connection "+ip_addr in open('/etc/nginx/'
                    ...
                    Log.info(self, "Setting up Nginx debug connection"
                             " for "+ip_addr)
                    KCShellExec.cmd_exec(self, "sed -i \"/events {{/a\\ \\ \\ "
        ...
        # stop global debug
            ...
            if "debug_connection " in open('/etc/nginx/nginx.conf',
                ...
                KCShellExec.cmd_exec(self, "sed -i \"/debug_connection.*/d\""
        ...
        # stop site specific debug
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if os.path.isfile(config_path):
                if KCShellExec.cmd_exec(self, "grep \"error.log debug\" {0}"
                                        .format(config_path)):
                    Log.info(self, "Stoping NGINX debug connection for {0}"
                             .format(self.app.pargs.site_name))
                    KCShellExec.cmd_exec(self, "sed -i \"s/error.log debug;/"
                                         "error.log;/\" {0}"
                                         .format(config_path))
                    self.trigger_nginx = True
    ...
    def debug_php(self):
        ...
        if (self.app.pargs.php == 'on' and not self.app.pargs.site_name):
            if not (KCShellExec.cmd_exec(self, "sed -n \"/upstream php"
                                               "{/,/}/p \" /etc/nginx/"
                ...
                # Enable xdebug
                KCFileUtils.searchreplace(self, "/etc/{0}/"
        ...
        # PHP global debug stop
            ...
            if KCShellExec.cmd_exec(self, " sed -n \"/upstream php {/,/}/p\" "
    ...
    def debug_fpm(self):
        ...
        if (self.app.pargs.fpm == 'on' and not self.app.pargs.site_name):
            if not KCShellExec.cmd_exec(self, "grep \"log_level = debug\" "
                                              "/etc/{0}/"
        ...
        # PHP5-FPM stop global debug
            ...
            if KCShellExec.cmd_exec(self, "grep \"log_level = debug\" "
                                          "/etc/{0}/fpm/php-fpm.conf".format("php/7.2")):
    ...
    def debug_php73(self):
        ...
        if (self.app.pargs.php73 == 'on' and not self.app.pargs.site_name):
            if not (KCShellExec.cmd_exec(self, "sed -n \"/upstream php73"
                                               "{/,/}/p \" /etc/nginx/"
                                               "conf.d/upstream.conf "
                                               "| grep 9173")):
                ...
                # Enable xdebug
                KCFileUtils.searchreplace(self, "/etc/php/7.3/mods-available/"
                                          "xdebug.ini",
                                          ";zend_extension",
                                          "zend_extension")
        ...
        # PHP global debug stop
            ...
            if KCShellExec.cmd_exec(self, " sed -n \"/upstream "
                                    "php73 {/,/}/p\" "
                                          "/etc/nginx/conf.d/upstream.conf "
                                          "| grep 9173"):
    ...
    def debug_mysql(self):
        ...
        if (self.app.pargs.mysql == 'on' and not self.app.pargs.site_name):
            if not KCShellExec.cmd_exec(self, "mysql -e \"show variables like"
                                              " \'slow_query_log\';\" | "
                                              "grep ON"):
                Log.info(self, "Setting up MySQL slow log")
                KCMysql.execute(self, "set global slow_query_log = "
                                      "\'ON\';")
                KCMysql.execute(self, "set global slow_query_log_file = "
                                      "\'/var/log/mysql/mysql-slow.log\';")
                KCMysql.execute(self, "set global long_query_time = 2;")
                KCMysql.execute(self, "set global log_queries_not_using"
                                      "_indexes = \'ON\';")
            else:
        ...
        # MySQL stop global debug
            ...
            if KCShellExec.cmd_exec(self, "mysql -e \"show variables like \'"
                                    "slow_query_log\';\" | grep ON"):
                Log.info(self, "Disabling MySQL slow log")
                KCMysql.execute(self, "set global slow_query_log = \'OFF\';")
                KCMysql.execute(self, "set global slow_query_log_file = \'"
                                "/var/log/mysql/mysql-slow.log\';")
                KCMysql.execute(self, "set global long_query_time = 10;")
                KCMysql.execute(self, "set global log_queries_not_using_index"
                                "es = \'OFF\';")
                KCShellExec.cmd_exec(self, "crontab -l | sed \'/#WordOps "
                                     "start/,/#KeepCloud end/d\' | crontab -")
            else:
    ...
    def debug_wp(self):
        ...
        if (self.app.pargs.wp == 'on' and self.app.pargs.site_name):
            wp_config = ("{0}/{1}/wp-config.php"
                         .format(KCVar.wo_webroot,
                                 self.app.pargs.site_name))
            webroot = "{0}{1}".format(KCVar.wo_webroot,
                                      self.app.pargs.site_name)
            ...
            if os.path.isfile(wp_config):
                if not KCShellExec.cmd_exec(self, "grep \"\'WP_DEBUG\'\" {0} |"
                                            " grep true".format(wp_config)):
                    Log.info(self, "Starting WordPress debug")
                    open("{0}/htdocs/wp-content/debug.log".format(webroot),
                         encoding='utf-8', mode='a').close()
                    KCShellExec.cmd_exec(self, "chown {1}: {0}/htdocs/wp-"
                                         "content/debug.log"
                                         "".format(webroot,
                                                   KCVar.wo_php_user))
                    KCShellExec.cmd_exec(self, "sed -i \"s/define(\'WP_DEBUG\'"
                                         ".*/define(\'WP_DEBUG\', true);\\n"
                                         "define(\'WP_DEBUG_DISPLAY\', false);"
                                         "\\ndefine(\'WP_DEBUG_LOG\', true);"
                                         "\\ndefine(\'SAVEQUERIES\', true);/\""
                                         " {0}".format(wp_config))
                    KCShellExec.cmd_exec(self, "cd {0}/htdocs/ && wp"
                                         " plugin --allow-root install "
                                         "developer query-monitor"
                                         .format(webroot))
                    KCShellExec.cmd_exec(self, "chown -R {1}: {0}/htdocs/"
                                         "wp-content/plugins"
                                         .format(webroot,
                                                 KCVar.wo_php_user))
    ...
    def debug_rewrite(self):
        ...
        if (self.app.pargs.rewrite == 'on' and not self.app.pargs.site_name):
            if not KCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" "
                                        "/etc/nginx/nginx.conf"):
                Log.info(self, "Setting up Nginx rewrite logs")
                KCShellExec.cmd_exec(self, "sed -i \'/http {/a \\\\t"
        ...
        # Stop Nginx rewrite debug globally
            ...
            if KCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" "
                                    "/etc/nginx/nginx.conf"):
                Log.info(self, "Disabling Nginx rewrite logs")
                KCShellExec.cmd_exec(self, "sed -i \"/rewrite_log.*/d\""
                                     " /etc/nginx/nginx.conf")
                self.trigger_nginx = True
            else:
        ...
        # Start Nginx rewrite for site
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if not KCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" {0}"
                                        .format(config_path)):
                Log.info(self, "Setting up Nginx rewrite logs for {0}"
                         .format(self.app.pargs.site_name))
                KCShellExec.cmd_exec(self, "sed -i \"/access_log/i \\\\\\t"
                                     "rewrite_log on;\" {0}"
                                     .format(config_path))
                self.trigger_nginx = True
            else:
            ...
            if ('{0}{1}/logs/error.log'.format(KCVar.wo_webroot,
                                               self.app.pargs.site_name)
                    not in self.msg):
                self.msg = self.msg + ['{0}{1}/logs/error.log'
                                       .format(KCVar.wo_webroot,
        ...
        # Stop Nginx rewrite for site
            ...
            config_path = ("/etc/nginx/sites-available/{0}"
                           .format(self.app.pargs.site_name))
            if KCShellExec.cmd_exec(self, "grep \"rewrite_log on;\" {0}"
                                    .format(config_path)):
                Log.info(self, "Disabling Nginx rewrite logs for {0}"
                         .format(self.app.pargs.site_name))
                KCShellExec.cmd_exec(self, "sed -i \"/rewrite_log.*/d\" {0}"
                                     .format(config_path))
                self.trigger_nginx = True
            else:
    ...
    def signal_handler(self, app, signal, frame):
        ...
        if self.app.pargs.mysql:
            ...
            if KCVar.wo_mysql_host == "localhost":
                self.app.pargs.mysql = 'off'
                self.debug_mysql()
            else:
                Log.warn(self, "Remote MySQL found, Keepcloud does not support "
        ...
        if self.trigger_php:
            if KCAptGet.is_installed(self, 'php7.2-fpm'):
                KCService.reload_service(self, 'php7.2-fpm')
            if KCAptGet.is_installed(self, 'php7.3-fpm'):
                KCService.reload_service(self, 'php7.3-fpm')
        self.app.close(0)
    ...
    def default(self):
        ...
        if ((not self.app.pargs.nginx) and (not self.app.pargs.php) and
            ...
            if self.app.pargs.stop or self.app.pargs.start:
                print("--start/stop option is deprecated since kc v2.0")
        ...
        if self.app.pargs.interval:
            ...
            try:
                if not KCShellExec.cmd_exec(self, "crontab -l | grep "
                                            "'kc debug --import-slow-log'"):
                    if not cron_time == 0:
                        Log.info(self, "setting up crontab entry,"
                                 " please wait...")
                        KCShellExec.cmd_exec(self, "/bin/bash -c \"crontab -l "
                                             "2> /dev/null | {{ cat; echo -e"
                                             " \\\"#KeepCloud start MySQL "
                                             "slow log \\n*/{0} * * * * "
                                             "/usr/local/bin/kc debug"
                                             " --import-slow-log\\n"
                                             "#Keepcloud end MySQL slow log"
                                             "\\\"; }} | crontab -\""
                                             .format(cron_time))
                else:
                    if not cron_time == 0:
        ...
        if self.trigger_php:
            if KCAptGet.is_installed(self, 'php7.2-fpm'):
                KCService.restart_service(self, 'php7.2-fpm')
            if KCAptGet.is_installed(self, 'php7.3-fpm'):
                KCService.restart_service(self, 'php7.3-fpm')
    ...
    def import_slow_log(self):
        ...
        if os.path.isdir("{0}22222/htdocs/db/anemometer"
            ...
            if os.path.isfile("/var/log/mysql/mysql-slow.log"):
                ...
                try:
                    KCShellExec.cmd_exec(self, "pt-query-digest --user={0} "
                                         "--password={1} "
            ...
            Log.error(self, "Anemometer is not installed." +
                      ...
                      Log.BOLD + "\n `kc stack install --utils`" +
                      Log.ENDC)


def load(app):
    # register the plugin class.. this only happens if the plugin is enabled
    app.handler.register(KCDebugController)
    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', kc_debug_hook)
