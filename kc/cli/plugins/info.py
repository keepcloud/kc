...
def kc_info_hook(app):
    pass


class KCInfoController(CementBaseController):
    class Meta:
        label = 'info'
        stacked_on = 'base'
        stacked_type = 'nested'
    ...
    def default(self):
        ...
        if pargs.nginx:
            if ((not KCAptGet.is_installed(self, 'nginx-custom')) and
        ...
        if pargs.mysql:
            if KCShellExec.cmd_exec(self, "/usr/bin/mysqladmin ping"):
                self.info_mysql()
            else:
...
def load(app):
    ...
    app.handler.register(KCInfoController)

    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', kc_info_hook)
