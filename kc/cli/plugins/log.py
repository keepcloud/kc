...
from kc.core.sendmail import KCSendMail
...
class KCLogController(CementBaseController):
    class Meta:
        label = 'log'
        description = 'Perform operations on Nginx, PHP and MySQL log files'
        stacked_on = 'base'
        stacked_type = 'nested'
        usage = "kc log [<site_name>] [options]"
...
class KCLogShowController(CementBaseController):
    class Meta:
        label = 'show'
        ...
        usage = "kc log show [<site_name>] [options]"
                ...
                if os.path.isdir('{0}/htdocs/wp-content'.format(webroot)):
                    if not os.path.isfile('{0}/logs/debug.log'
                        ...
                        if not os.path.isfile('{0}/htdocs/wp-content/debug.log'
                            ...
                            KCShellExec.cmd_exec(self, "chown {1}: {0}/htdocs/"
                    ...
                    KCFileUtils.create_symlink(self, ["{0}/htdocs/wp-content/"
                                                      "debug.log"
                                                      .format(webroot),
                                                      '{0}/logs/debug.log'
                                                      .format(webroot)])
...
class KCLogResetController(CementBaseController):
    class Meta:
        label = 'reset'
        ...
        usage = "kc log reset [<site_name>] [options]"
    ...
    def default(self):
        ...
        if self.app.pargs.slow_log_db:
            if os.path.isdir("/var/www/22222/htdocs/db/anemometer"):
                ...
                KCMysql.execute(self, "TRUNCATE TABLE  "
...
class KCLogGzipController(CementBaseController):
    class Meta:
        label = 'gzip'
        ...
        usage = "kc log gzip [<site_name>] [options]"
...
class KCLogMailController(CementBaseController):
    class Meta:
        label = 'mail'
        ...
        usage = "kc log mail [<site_name>] [options]"
    ...
    def default(self):
        ...
        if self.app.pargs.site_name:
            webroot = "{0}{1}".format(KCVar.kc_webroot,
                                      self.app.pargs.site_name)
        ...
        for tomail in self.app.pargs.to:
            ...
            KCSendMail("wordops", tomail[0], "{0} Log Files"
                       .format(KCVar.wo_fqdn),
                       "Hi,\n  The requested logfiles are attached."
                       "\n\nBest regards,\nYour KeepCloud worker",
                       files=mail_list, port=25, isTls=False)


def load(app):
    # register the plugin class.. this only happens if the plugin is enabled
    app.handler.register(KCLogController)
    app.handler.register(KCLogShowController)
    app.handler.register(KCLogResetController)
    app.handler.register(KCLogGzipController)
    app.handler.register(KCLogMailController)
    # register a hook (function) to run after arguments are parsed.
    app.hook.register('post_argument_parsing', kc_log_hook)
