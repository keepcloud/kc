...
from kc.core.random import RANDOM
...
def kc_secure_hook(app):
    pass


class KCSecureController(CementBaseController):
    class Meta:
        label = 'secure'
        stacked_on = 'base'
        stacked_type = 'nested'
    ...
    def secure_auth(self):
        ...
        if not pargs.user_input:
            username = input("Provide HTTP authentication user "
                             "name [{0}] :".format(KCVar.kc_user))
            pargs.user_input = username
            if username == "":
                pargs.user_input = KCVar.kc_user
        ...
        Log.debug(self, "printf username:"
                  ...
                  "password 2> /dev/null)\n\""
                  "> /etc/nginx/htpasswd-kc 2>/dev/null")
        KCShellExec.cmd_exec(self, "printf \"{username}:"
                             "$(openssl passwd -crypt "
                             "{password} 2> /dev/null)\n\""
                             "> /etc/nginx/htpasswd-kc 2>/dev/null"
    ...
    def secure_port(self):
        ...
        if pargs.user_input:
            while ((not pargs.user_input.isdigit()) and
                ...
                pargs.user_input = input("KeepCloud "
                                         "admin port [22222]:")
        else:
            port = input("KeepCloud admin port [22222]:")
            if port == "":
                port = 22222
        ...
        data = dict(release=KCVar.wo_version,
    ...
    def secure_ip(self):
        ...
        for ip_addr in user_ip:
            if not ("exist_ip_address " + ip_addr in open('/etc/nginx/common/'
                                                          'acl.conf').read()):
                KCShellExec.cmd_exec(self, "sed -i "
    ...
    def secure_ssh(self):
        ...
        if os.path.isfile('/etc/ssh/sshd_config'):
            ...
            if pargs.allowpassword:
                kc_allowpassword = 'yes'
            else:
                kc_allowpassword = 'no'
            data = dict(sshport=current_ssh_port, allowpass=kc_allowpassword,
                        user=sudo_user)
            KCTemplate.deploy(self, '/etc/ssh/sshd_config',
                              'sshd.mustache', data)
            KCGit.add(self, ["/etc/ssh"],
                      msg="Adding changed SSH port into Git")
            if not KCService.restart_service(self, 'ssh'):
    ...
    def secure_ssh_port(self):
        ...
        if not pargs.user_input:
            ...
            while (not port.isdigit()) and (port != "") and (not port < 65536):
                ...
                port = input("Server SSH port [22]:")
            pargs.user_input = port
        if KCFileUtils.grepcheck(self, '/etc/ssh/sshd_config', '#Port'):
            KCShellExec.cmd_exec(self, "sed -i \"s/#Port.*/Port "
...
def load(app):
    app.handler.register(KCSecureController)
    app.hook.register('post_argument_parsing', kc_secure_hook)
