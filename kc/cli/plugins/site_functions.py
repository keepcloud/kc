...
from kc.cli.plugins.sitedb import getSiteInfo
from kc.cli.plugins.stack import KCStackController
from kc.cli.plugins.stack_pref import post_pref
...
from kc.core.mysql import (MySQLConnectionError, StatementExcecutionError,
                           KCMysql)
...
def setupdomain(self, data):
    ...
    kc_domain_name = data['site_name']
    kc_site_webroot = data['webroot']
    ...
    try:
        kc_site_nginx_conf = open('/etc/nginx/sites-available/{0}'
                                  .format(kc_domain_name), encoding='utf-8',
                                  mode='w')
        self.app.render((data), 'virtualconf.mustache',
                        out=kc_site_nginx_conf)
        kc_site_nginx_conf.close()
    except IOError as e:
    ...
    KCFileUtils.create_symlink(self, ['/etc/nginx/sites-available/{0}'
                                      .format(kc_domain_name),
    ...
    try:
        if not os.path.exists('{0}/htdocs'.format(kc_site_webroot)):
            os.makedirs('{0}/htdocs'.format(kc_site_webroot))
        if not os.path.exists('{0}/logs'.format(kc_site_webroot)):
            os.makedirs('{0}/logs'.format(kc_site_webroot))
        if not os.path.exists('{0}/conf/nginx'.format(kc_site_webroot)):
            os.makedirs('{0}/conf/nginx'.format(kc_site_webroot))

        KCFileUtils.create_symlink(self, ['/var/log/nginx/{0}.access.log'
                                          .format(kc_domain_name),
                                          '{0}/logs/access.log'
                                          .format(kc_site_webroot)])
        KCFileUtils.create_symlink(self, ['/var/log/nginx/{0}.error.log'
                                          .format(kc_domain_name),
                                          '{0}/logs/error.log'
                                          .format(kc_site_webroot)])
    except Exception as e:
        ...
        if (os.path.exists('{0}/htdocs'.format(kc_site_webroot)) and
                os.path.exists('{0}/logs'.format(kc_site_webroot))):
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        else:
...
def setupdatabase(self, data):
    kc_domain_name = data['site_name']
    kc_random_pass = (''.join(random.sample(string.ascii_uppercase +
                                            string.ascii_lowercase +
                                            string.digits, 24)))
    kc_replace_dash = kc_domain_name.replace('-', '_')
    kc_replace_dot = kc_replace_dash.replace('.', '_')
    kc_replace_underscore = kc_replace_dot.replace('_', '')
    if self.app.config.has_section('mysql'):
        ...
        kc_mysql_grant_host = self.app.config.get('mysql', 'grant-host')
    else:
        prompt_dbname = False
        prompt_dbuser = False
    ...
    if prompt_dbname == 'True' or prompt_dbname == 'true':
        try:
            kc_db_name = input('Enter the MySQL database name [{0}]: '
                               .format(kc_replace_dot))
        except EOFError:
            raise SiteError("Unable to input database name")

    if not kc_db_name:
        kc_db_name = (kc_replace_dot[0:32] + '_' + generate_8_random())

    if prompt_dbuser == 'True' or prompt_dbuser == 'true':
        try:
            kc_db_username = input('Enter the MySQL database user name [{0}]: '
                                   .format(kc_replace_dot))
            kc_db_password = getpass.getpass(prompt='Enter the MySQL database'
                                             ' password [{0}]: '
                                             .format(kc_random_pass))
        except EOFError:
            raise SiteError("Unable to input database credentials")

    if not kc_db_username:
        kc_db_username = (kc_replace_underscore[0:12] + generate_random())
    if not kc_db_password:
        kc_db_password = kc_random_pass

    # create MySQL database
    Log.info(self, "Setting up database\t\t", end='')
    Log.debug(self, "Creating database {0}".format(kc_db_name))
    try:
        if KCMysql.check_db_exists(self, kc_db_name):
            Log.debug(self, "Database already exists, Updating DB_NAME .. ")
            kc_db_name = (kc_db_name[0:32] + '_' + generate_8_random())
            kc_db_username = (kc_db_name[0:12] + generate_random())
    except MySQLConnectionError:
        raise SiteError("MySQL Connectivity problem occured")

    try:
        KCMysql.execute(self, "create database `{0}`"
                        .format(kc_db_name))
    ...
    # Create MySQL User
    Log.debug(self, "Creating user {0}".format(kc_db_username))
    Log.debug(self, "create user `{0}`@`{1}` identified by ''"
              .format(kc_db_username, kc_mysql_grant_host))
    try:
        KCMysql.execute(self,
                        "create user `{0}`@`{1}` identified by '{2}'"
                        .format(kc_db_username, kc_mysql_grant_host,
                                kc_db_password), log=False)
    except StatementExcecutionError:
        Log.info(self, "[" + Log.ENDC + Log.FAIL + "Failed" + Log.OKBLUE + "]")
    ...
    try:
        KCMysql.execute(self,
                        ...
                        "trigger on `{0}`.* to `{1}`@`{2}`"
                        .format(kc_db_name,
                                kc_db_username, kc_mysql_grant_host))
    except StatementExcecutionError:
        Log.info(self, "[" + Log.ENDC + Log.FAIL + "Failed" + Log.OKBLUE + "]")
    ...
    data['kc_db_name'] = kc_db_name
    data['kc_db_user'] = kc_db_username
    data['kc_db_pass'] = kc_db_password
    data['kc_db_host'] = KCVar.kc_mysql_host
    data['kc_mysql_grant_host'] = kc_mysql_grant_host
    return(data)
...
def setupwordpress(self, data, vhostonly=False):
    ...
    if self.app.config.has_section('wordpress'):
        ...
        kc_wp_user = self.app.config.get('wordpress', 'user')
        kc_wp_pass = self.app.config.get('wordpress', 'password')
        kc_wp_email = self.app.config.get('wordpress', 'email')
    else:
    ...
    if 'wp-user' in data.keys() and data['wp-user']:
        kc_wp_user = data['wp-user']
    if 'wp-email' in data.keys() and data['wp-email']:
        kc_wp_email = data['wp-email']
    if 'wp-pass' in data.keys() and data['wp-pass']:
        kc_wp_pass = data['wp-pass']

    Log.info(self, "Downloading WordPress \t\t", end='')
    KCFileUtils.chdir(self, '{0}/htdocs/'.format(kc_site_webroot))
    try:
        if KCShellExec.cmd_exec(self, "wp --allow-root core"
                                " download"):
            pass
        else:
    ...
    if not (data['kc_db_name'] and data['kc_db_user'] and data['kc_db_pass']):
        data = setupdatabase(self, data)
    if prompt_wpprefix == 'True' or prompt_wpprefix == 'true':
        try:
            kc_wp_prefix = input('Enter the WordPress table prefix [wp_]: ')
            while not re.match('^[A-Za-z0-9_]*$', kc_wp_prefix):
    ...
    if not data['multisite']:
        ...
        Log.debug(self, "/bin/bash -c \"{0} --allow-root "
                  ...
                  "--dbhost=\'{3}\' "
                  .format(data['kc_db_name'], kc_wp_prefix,
                          data['kc_db_user'], data['kc_db_host']) +
                  "--dbpass= "
                  "--extra-php<<PHP \n {0}\nPHP\""
                  .format("\n\ndefine(\'WP_DEBUG\', false);"))
        try:
            if KCShellExec.cmd_exec(self, "/bin/bash -c \"{0} --allow-root"
                                    .format(KCVar.kc_wpcli_path) +
                                    " config create " +
                                    "--dbname=\'{0}\' --dbprefix=\'{1}\' "
                                    "--dbuser=\'{2}\' --dbhost=\'{3}\' "
        ...
        Log.debug(self, "/bin/bash -c \"{0} --allow-root "
                  ...
                  .format(data['kc_db_name'],
                          kc_wp_prefix, data['kc_db_host']) +
                  "--dbuser=\'{0}\' --dbpass= "
                  "--extra-php<<PHP \n {1} {2} \nPHP\""
                  .format(data['kc_db_user'],
    ...
    wp_conf_variables = [
        ['WP_REDIS_PREFIX', '{0}:'.format(kc_domain_name)],
    ...
    # KCFileUtils.mvfile(self, os.getcwd()+'/wp-config.php',
    ...
    if not kc_wp_user:
        kc_wp_user = KCVar.kc_user
        while not kc_wp_user:
            Log.warn(self, "Username can have only alphanumeric"
                     "characters, spaces, underscores, hyphens,"
                     "periods and the @ symbol.")
            try:
                kc_wp_user = input('Enter WordPress username: ')
            except EOFError:
                raise SiteError("input WordPress username failed")
    if not kc_wp_pass:
        kc_wp_pass = kc_random_pass

    if not kc_wp_email:
        kc_wp_email = KCVar.kc_email
        while not kc_wp_email:
            try:
                kc_wp_email = input('Enter WordPress email: ')
            except EOFError:
                raise SiteError("input WordPress username failed")
    ...
    try:
        while not re.match(r"^[A-Za-z0-9\.\+_-]+@[A-Za-z0-9\._-]+\.[a-zA-Z]*$",
            ...
            kc_wp_email = input("Enter your email: ")
    except EOFError:
    ...
    if not data['multisite']:
        ...
        Log.debug(
            self, "{0} --allow-root core install "
                  ...
                  .format(data['site_name'], kc_wp_user) +
    ...
    try:
        KCShellExec.cmd_exec(self, " {0} --allow-root "
                             .format(KCVar.kc_wpcli_path) +
                             "rewrite structure "
                             "/%postname%/")
    except CommandExecutionError as e:
    ...
    if vhostonly:
        try:
            KCShellExec.cmd_exec(self, "/bin/bash -c \"{0} --allow-root "
                                 .format(KCVar.kc_wpcli_path) +
                                 "db clean --yes\"")
            KCFileUtils.chdir(self, '{0}'.format(kc_site_webroot))
            KCFileUtils.rm(self, "{0}/htdocs".format(kc_site_webroot))
            KCFileUtils.mkdir(self, "{0}/htdocs".format(kc_site_webroot))
            KCFileUtils.chown(self, "{0}/htdocs".format(kc_site_webroot),
                              'www-data', 'www-data')
        except CommandExecutionError:
            raise SiteError("Cleaning WordPress install failed")

    wp_creds = dict(wp_user=kc_wp_user, wp_pass=kc_wp_pass,
                    wp_email=kc_wp_email)
...
def setupwordpressnetwork(self, data):
    ...
    try:
        if KCShellExec.cmd_exec(self, 'wp --allow-root core multisite-convert'
                                ' --title=\'{0}\' {subdomains}'
...
def installwp_plugin(self, plugin_name, data):
    ...
    KCFileUtils.chdir(self, '{0}/htdocs/'.format(kc_site_webroot))
    try:
        KCShellExec.cmd_exec(self, "{0} plugin "
                             .format(KCVar.kc_wpcli_path) +
                             "--allow-root install "
...
def setwebrootpermissions(self, webroot):
    ...
    try:
        ...
        KCFileUtils.chown(self, webroot, KCVar.kc_php_user,
                          KCVar.kc_php_user, recursive=True)
    except Exception as e:
        Log.debug(self, str(e))
...
def sitebackup(self, data):
    kc_site_webroot = data['webroot']
    backup_path = kc_site_webroot + '/backup/{0}'.format(KCVar.kc_date)
    if not KCFileUtils.isexist(self, backup_path):
        KCFileUtils.mkdir(self, backup_path)
    Log.info(self, "Backup location : {0}".format(backup_path))
    KCFileUtils.copyfile(self, '/etc/nginx/sites-available/{0}'
    ...
    if data['currsitetype'] in ['html', 'php', 'php72', 'php74',
        ...
        if not data['wp']:
            ...
            KCFileUtils.copyfiles(self, kc_site_webroot +
                                  '/htdocs', backup_path + '/htdocs')
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        else:
            Log.info(self, "Backing up Webroot \t\t", end='')
            KCFileUtils.mvfile(self, kc_site_webroot + '/htdocs', backup_path)
            Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
    ...
    if not configfiles:
        ...
        if data['currsitetype'] in ['mysql']:
            ...
            Log.debug(self, "Searching wp-config.php in {0}/htdocs/ "
                      .format(kc_site_webroot))
            configfiles = glob.glob(kc_site_webroot + '/htdocs/wp-config.php')

    # if configfiles and KCFileUtils.isexist(self, configfiles[0]):
    #     kc_db_name = (KCFileUtils.grep(self, configfiles[0],
    #                   'DB_NAME').split(',')[1]
    ...
    if data['kc_db_name']:
        ...
        try:
            ...
            raise SiteError("mysqldump failed to backup database")
        Log.info(self, "[" + Log.ENDC + "Done" + Log.OKBLUE + "]")
        # move wp-config.php/kc-config.php to backup
        if data['currsitetype'] in ['mysql', 'proxy']:
            if data['php73'] is True and not data['wp']:
                KCFileUtils.copyfile(self, configfiles[0], backup_path)
            else:
                KCFileUtils.mvfile(self, configfiles[0], backup_path)
        else:
            KCFileUtils.copyfile(self, configfiles[0], backup_path)
...
def site_package_check(self, stype):
    ...
    if stype in ['html', 'proxy', 'php', 'php72', 'mysql', 'wp', 'wpsubdir',
        ...
        if not (KCAptGet.is_installed(self, 'nginx-custom') or
                KCAptGet.is_installed(self, 'nginx-mainline')):
            # check if Server has nginx-plus installed
            if KCAptGet.is_installed(self, 'nginx-plus'):
                # do something
                # do post nginx installation configuration
                Log.info(self, "NGINX PLUS Detected ...")
                apt = ["nginx-plus"] + KCVar.kc_nginx
                # apt_packages = apt_packages + KCVar.kc_nginx
                post_pref(self, apt, packages)
            elif KCAptGet.is_installed(self, 'nginx'):
                Log.info(self, "KeepCloud detected a previously"
                               "installed Nginx package. "
                               "It may or may not have required modules. "
                               "\nIf you need help, please create an issue at "
                               "https://github.com/KeepCloud/kc/issues/ \n")
                apt = ["nginx"] + KCVar.kc_nginx
                # apt_packages = apt_packages + KCVar.kc_nginx
                post_pref(self, apt, packages)
    ...
    if ((pargs.php and pargs.php73) or (pargs.php and pargs.php74) or
        ...
        Log.error(
            self, "Error: two different PHP versions cannot be "
                  "combined within the same Keepcloud site")
    ...
    if ((not pargs.php72) and (not pargs.php73) and (not pargs.php74) and
        ...
        if not (KCAptGet.is_installed(self, php_check)):
            apt_packages = apt_packages + php_to_setup + KCVar.kc_php_extra

    if pargs.php72 and stype in ['php72', 'mysql', 'wp',
                                          'wpsubdir', 'wpsubdomain']:
        Log.debug(self, "Setting apt_packages variable for PHP 7.2")
        if not KCAptGet.is_installed(self, 'php7.2-fpm'):
            apt_packages = apt_packages + KCVar.kc_php72 + KCVar.kc_php_extra

    if pargs.php73 and stype in ['php73', 'mysql', 'wp',
                                          'wpsubdir', 'wpsubdomain']:
        Log.debug(self, "Setting apt_packages variable for PHP 7.3")
        if not KCAptGet.is_installed(self, 'php7.3-fpm'):
            apt_packages = apt_packages + KCVar.kc_php73 + KCVar.kc_php_extra
    ...
    if pargs.php74 and stype in ['php74', 'mysql', 'wp',
        ...
        if not KCAptGet.is_installed(self, 'php7.4-fpm'):
            apt_packages = apt_packages + KCVar.kc_php74 + KCVar.kc_php_extra
    ...
    if pargs.php80 and stype in ['php80', 'mysql', 'wp',
        ...
        if not KCAptGet.is_installed(self, 'php8.0-fpm'):
            apt_packages = apt_packages + KCVar.kc_php80 + KCVar.kc_php_extra
    ...
    if pargs.php81 and stype in ['php81', 'mysql', 'wp',
        ...
        if not KCAptGet.is_installed(self, 'php8.1-fpm'):
            apt_packages = apt_packages + KCVar.kc_php81 + KCVar.kc_php_extra

    if stype in ['mysql', 'wp', 'wpsubdir', 'wpsubdomain']:
        Log.debug(self, "Setting apt_packages variable for MySQL")
        if not KCShellExec.cmd_exec(self, "/usr/bin/mysqladmin ping"):
...
def updatewpuserpassword(self, kc_domain, kc_site_webroot):

    kc_wp_user = ''
    kc_wp_pass = ''
    KCFileUtils.chdir(self, '{0}/htdocs/'.format(kc_site_webroot))

    if not KCShellExec.cmd_exec(self, "wp --allow-root core"
                                " is-installed"):
        # Exit if kc_domain is not wordpress install
        Log.error(self, "{0} does not seem to be a WordPress site"
                  .format(kc_domain))

    try:
        kc_wp_user = input("Provide WordPress user name [admin]: ")
    except Exception as e:
        Log.debug(self, str(e))
    ...
    if kc_wp_user == "?":
        ...
        try:
            KCShellExec.cmd_exec(self, "wp --allow-root user list "
    ...
    try:
        is_user_exist = KCShellExec.cmd_exec(self, "wp --allow-root user list "
    ...
    if is_user_exist:
        try:
            kc_wp_pass = getpass.getpass(prompt="Provide password for "
                                         "{0} user: "
                                         .format(kc_wp_user))

            while not kc_wp_pass:
                kc_wp_pass = getpass.getpass(prompt="Provide password for "
                                             "{0} user: "
                                             .format(kc_wp_user))
        except Exception as e:
        ...
        try:
            KCShellExec.cmd_exec(self, "wp --allow-root user update {0}"
                                 "  --user_pass={1}"
                                 .format(kc_wp_user, kc_wp_pass))
        except CommandExecutionError as e:
            Log.debug(self, str(e))
...
def logwatch(self, logfiles):
    ...
    import base64
    import time
    from kc.core.logwatch import LogWatcher
...
def generate_random_pass():
    kc_random10 = (''.join(random.sample(string.ascii_uppercase +
                                         string.ascii_lowercase +
                                         string.digits, 24)))
    return kc_random10
...
def generate_8_random():
    kc_random8 = (''.join(random.sample(string.ascii_uppercase +
                                        string.ascii_lowercase +
                                        string.digits, 8)))
    return kc_random8
...
def deleteDB(self, dbname, dbuser, dbhost, exit=True):
    try:
        ...
        try:
            if KCMysql.check_db_exists(self, dbname):
        ...
        if dbuser != 'root':
            ...
            try:
                KCMysql.execute(self, "flush privileges")
            except StatementExcecutionError as e:
                Log.debug(self, str(e))
                Log.debug(self, "drop database failed")
                Log.info(self, "Database {0} not dropped".format(dbname))
...
def removeNginxConf(self, domain):
    if os.path.isfile('/etc/nginx/sites-available/{0}'
        ...
        KCFileUtils.rm(self, '/etc/nginx/sites-enabled/{0}'
                       .format(domain))
        KCFileUtils.rm(self, '/etc/nginx/sites-available/{0}'
                       .format(domain))
...
def copyWildcardCert(self, kc_domain_name, kc_root_domain):

    if os.path.isfile("/var/www/{0}/conf/nginx/ssl.conf"
                      .format(kc_root_domain)):
        try:
            if not os.path.isdir("/etc/letsencrypt/shared"):
                KCFileUtils.mkdir(self, "/etc/letsencrypt/shared")
            if not os.path.isfile("/etc/letsencrypt/shared/{0}.conf"
                                  .format(kc_root_domain)):
                KCFileUtils.copyfile(self, "/var/www/{0}/conf/nginx/ssl.conf"
                                     .format(kc_root_domain),
                                     "/etc/letsencrypt/shared/{0}.conf"
                                     .format(kc_root_domain))
            KCFileUtils.create_symlink(self, ["/etc/letsencrypt/shared/"
                                              "{0}.conf"
                                              .format(kc_root_domain),
...
def renewLetsEncrypt(self, kc_domain_name):
    ...
    if not ssl:
        Log.error(self, "ERROR : Let's Encrypt certificate renewal FAILED!",
                  False)
        if (SSL.getexpirationdays(self, kc_domain_name) > 0):
            Log.error(self, "Your current certificate will expire within " +
                      str(SSL.getexpirationdays(self, kc_domain_name)) +
                      " days.", False)
        else:
            Log.error(self, "Your current certificate already expired!", False)

        # KCSendMail("keepcloud@{0}".format(kc_domain_name), kc_wp_email,
        #  "[FAIL] HTTPS cert renewal {0}".format(kc_domain_name),
        #          "Hi,\n\nHTTPS certificate renewal for https://{0}
        # was unsuccessful.".format(kc_domain_name) +
        #           "\nPlease check the KeepCloud log for reason
        # The current expiry date is : " +
        #           str(SSL.getExpirationDate(self, kc_domain_name)) +
        #           "\n\nFor support visit https://keepcloud.io/support .
        # \n\nBest regards,\nYour Keepcloud Worker", files=mail_list,
        #           port=25, isTls=False)
        Log.error(self, "Check the KC log for more details "
                  "`tail /var/log/kc/keepcloud.log`")

    KCGit.add(self, ["/etc/letsencrypt"],
              msg="Adding letsencrypt folder")
    # KCSendMail("keepcloud@{0}".format(kc_domain_name), kc_wp_email,
    # "[SUCCESS] Let's Encrypt certificate renewal {0}".format(kc_domain_name),
    #           "Hi,\n\nYour Let's Encrypt certificate has been renewed for
    # https://{0} .".format(kc_domain_name) +
...
def setuprocketchat(self):
    if ((not KCVar.kc_platform_codename == 'bionic') and
            (not KCVar.kc_platform_codename == 'xenial')):
        Log.info(self, "Rocket.chat is only available on Ubuntu 16.04 "
                 "& 18.04 LTS")
        return False
    else:
        if not KCAptGet.is_installed(self, 'snapd'):
            KCAptGet.install(self, ["snapd"])
        if KCShellExec.cmd_exec(self, "snap install rocketchat-server"):
            return True
        return False
