...
class KCGit:
    ...
    def add(self, paths, msg="Intializating"):
        ...
        for path in paths:
            global git
            kcgit = git.bake("-C", "{0}".format(path))
            if os.path.isdir(path):
                if not os.path.isdir(path + "/.git"):
                    try:
                        Log.debug(self, "KCGit: git init at {0}"
                                  .format(path))
                        kcgit.init(path)
                ...
                status = kcgit.status("-s")
                if len(status.splitlines()) > 0:
                    try:
                        Log.debug(self, "KCGit: git commit at {0}"
                                  .format(path))
                        kcgit.add("--all")
                        kcgit.commit("-am {0}".format(msg))
                    except ErrorReturnCode as e:
                        Log.debug(self, "{0}".format(e))
                        Log.error(self, "Unable to git commit at {0} "
                                  .format(path))
            else:
                Log.debug(self, "KCGit: Path {0} not present".format(path))
    ...
    def checkfilestatus(self, repo, filepath):
        ...
        """
        global git
        kcgit = git.bake("-C", "{0}".format(repo))
        status = kcgit.status("-s", "{0}".format(filepath))
        if len(status.splitlines()) > 0:
            return True
        else:
            return False
    ...
    def rollback(self, paths, msg="Rolling-Back"):
        ...
        for path in paths:
            ...
            if os.path.isdir(path):
                ...
                try:
                    Log.debug(
                        self, "KCGit: git stash --include-untracked at {0}"
                              .format(path))
                    kcgit.stash("push", "--include-untracked", "-m {0}"
                                .format(msg))
                except ErrorReturnCode as e:
                    Log.debug(self, "{0}".format(e))
    ...
    def clone(self, repo, path, branch='master'):
        ...
        if not os.path.exists('{0}'.format(path)):
            ...
            Log.debug(self, "KCGit: Path {0} already exist".format(path))
