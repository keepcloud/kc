...
class KCMysql():
    ...
    def execute(self, statement, errormsg='', log=True):
        ...
        # & Execute MySQL query
        connection = KCMysql.connect(self)
    ...
    def backupAll(self, fulldump=False):
        ...
        try:
            Log.info(self, "Backing up database at location: "
                     "/var/lib/kc-backup/mysql")
            # Setup Nginx common directory
            if not os.path.exists('/var/lib/kc-backup/mysql'):
                Log.debug(self, 'Creating directory'
                          '/var/lib/kc-backup/mysql')
                os.makedirs('/var/lib/kc-backup/mysql')
            if not fulldump:
                ...
                for dbs in db:
                    ...
                    p2 = subprocess.Popen(
                        "/usr/bin/zstd -c > "
                        "/var/lib/kc-backup/mysql/{0}{1}.sql.zst"
                        .format(dbs, KCVar.kc_date),
                ...
                p2 = subprocess.Popen(
                    "/usr/bin/zstd -c > "
                    "/var/lib/kc-backup/mysql/fulldump-{0}.sql.zst"
                    .format(KCVar.kc_date),
                    stdin=p1.stdout, shell=True)
                p1.stdout.close()
                output = p1.stderr.read()
                p1.wait()
    ...
    def check_db_exists(self, db_name):
        try:
            if KCMysql.dbConnection(self, db_name):
                return True
