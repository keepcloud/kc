"""KeepCloud package installation using apt-get module."""
import subprocess
import sys
import os
...
class KCAptGet():
    ...
    def update(self):
        ...
        """
        try:
            with open('/var/log/kc/keepcloud.log', 'a') as f:
                ...
                if proc.returncode == 0:
                    ...
                    Log.error(self, "Check the KeepCloud log for more details "
        ...
        try:
            with open('/var/log/kc/keepcloud.log', 'a') as f:
                if repo_url is not None:
                    KCRepo.add(self, repo_url=repo_url)
                if repo_key is not None:
                    KCRepo.add_key(self, repo_key)
                proc = subprocess.Popen(
            ...
            if proc.returncode == 0:
                ...
                if repo_url is not None:
                    KCRepo.remove(self, repo_url=repo_url)
                return False
        except Exception as e:
            Log.debug(self, "{0}".format(e))
